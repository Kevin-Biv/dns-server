...
public class DNSServer {
    ...
    private DatagramPacket handleQuery(DNSMessage query) {
        ...
        boolean inZone = false;  // Changed to false by default
        ...
        ArrayList<DNSRecord> cacheRecord;

        if (records.size() == 0) {
            // Check cache first
            cacheRecord = cache.lookup(query.getQuestionName(), query.getQuestionType(), query.getQuestionClass());

            if (cacheRecord != null && !cacheRecord.isEmpty()) {
                // Found in cache, create response
                var reply = new DNSMessage(query, cacheRecord, inZone);
                System.out.println("Reply from cache to " + query.getPacket().getSocketAddress());
                System.out.println(reply);
                return new DatagramPacket(
                        reply.getData(),
                        reply.getDataLength(),
                        query.getPacket().getSocketAddress()
                        );
            }

            // If not in cache, forward to upstream server
            System.out.println("\nForwarding Query to " + nextServer + ":" + nextServerPort);
            System.out.println(query);
        ...
        // If we have records in our zone
        inZone = true;
    ...
    private DatagramPacket handleReply(DNSMessage reply) {
        ...
        System.out.println("\nReply received from " + reply.getPacket().getSocketAddress());
        System.out.println(reply);
        ...
        if (originalQuery != null) {
            ...
            cache.addRecords(
                    originalQuery.getQuestionName(), 
                    originalQuery.getQuestionType(), 
                    originalQuery.getQuestionClass(), 
                    reply.getAnswers()
                    );
            ...
            System.out.println("\nForwarding Reply to " + originalQuery.getPacket().getSocketAddress());
            System.out.println(reply);
            ...
            return new DatagramPacket(
                    reply.getData(),
                    reply.getDataLength(),
                    originalQuery.getPacket().getSocketAddress()
                    );
        }
        return null;
    }
